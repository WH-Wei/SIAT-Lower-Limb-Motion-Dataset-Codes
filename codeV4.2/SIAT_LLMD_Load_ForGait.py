...
flag_emg_standardization = 0
flag_get_emg_features_from_windows = 1
flag_features_standardization = 0

# path setup
path_data_set_folder_global = ".../SIAT_LLMD/"  # Don't miss '/' in the end
path_save_results_to_folder = "....save_folder_path...."
path_input_data_folder = '....save_folder_path..../emg_filtered/20211108172504/'  # If above flag = 0 happened, this path should be given!
...
# emg data setup
# movements_list_global: 'WAK', 'UPS', 'DNS'
subjects_list_global = list(range(1, 41, 1))

movements_list_global = ['WAK']
gait_period_global = ['HS-MSF', 'MSF-MSE', 'MSE-TO', 'TO-MWF', 'MWF-HS']

# movements_list_global = ['UPS']
# gait_period_global = ['HS-TO', 'TO-MWF', 'MWF-HS']
#
# movements_list_global = ['DNS']
# gait_period_global = ['HS-TO', 'TO-MWF', 'MWF-HS']
...
signal_processing_setup_global = {'method': ['baseline', 'wavelet_packet', 'butter_worth'],
                                  'butter_worth_low_cut': 15, 'butter_worth_high_cut': 400, 'butter_worth_fs': 1920, 'butter_worth_order': 7,
                                  'wavelet_packet_threshold': 0.08, 'wavelet_packet_threshold_mode': 'soft', 'wavelet_packet_wavelet_type': 'db7', 'wavelet_packet_dev_level': 8,
...
# example:
#       target_detail = ['subject', 'group', 'examples', 'channels']
...
EMG_standardization_setup_global = {'method': 'max_abs', 'target_detail': ['subject'], 'across_what': 'col', 'setup': {'norm': 'l1'}}
features_standardization_setup_global = {'method': 'positive_negative_one', 'target_detail': ['subject', 'channels'], 'across_what': 'all', 'setup': {'norm': 'l1'}}
...
# other setup
multi_progress_global = -1  # 0 : close multi-progress      -1 : create all progress        number>1 : create number>1 progress
...
def get_all_raw_data_path(path_data_set_folder, subjects_list, movements_list):
    ...
    for subjects_num in range(subjects_quantity):
        path_subject_raw_label_folder = path_data_set_folder + "Sub" + str(subjects_list[subjects_num]) + "/Labels/"
        path_subject_raw_data_folder = path_data_set_folder + "Sub" + str(subjects_list[subjects_num]) + "/Data/"
...
def get_emg_window_data(emg_data, label_status, label_group, window_size=150, window_overlap_size=50):
    ...
    counter = 0
    begin_group = -1
    label_status_begin = -1
    data_window_data = []
    data_window_group = []
    data_window_num_label = []
    data_window_OneHot_label = []

    for data_check in range(data_length):
        if label_status_begin == -1:
        ...
        if label_status[data_check] == label_status_begin:
            ...
            if counter == window_size:
                ...
                if len(data_window_data):
                    data_window_data = data_window_data + [target_data]
                    data_window_group = data_window_group + [begin_group]
                    data_window_num_label = data_window_num_label + [num_label_list_global[label_status_begin - 1]]
                    data_window_OneHot_label = data_window_OneHot_label + [OneHot_label_list_global[label_status_begin - 1]]

                else:
                    data_window_data = [target_data]
                    data_window_group = [begin_group]
                    data_window_num_label = [num_label_list_global[label_status_begin - 1]]
                    data_window_OneHot_label = [OneHot_label_list_global[label_status_begin - 1]]
    ...
    return data_window_data, data_window_group, data_window_num_label, data_window_OneHot_label
...
def get_data_information(path_data_set_folder, subjects_list, movements_list):
...
def get_emg_data_from_one_data(single_data_inform):
    ...
    emg_data = {'emg_data': raw_data_emg, 'time_data': raw_label_time,
...
def get_emg_windows_from_one_data(data_path):
    ...
    label_status = np.int8(list(data['label_status'][0]))
    ...
    data_emg, data_group, data_num_label, data_OneHot_label = get_emg_window_data(emg_data=data_emg, label_status=label_status, label_group=label_group, window_size=window_size_global,
                                                                                  window_overlap_size=window_overlap_size_global)
    data_angle, useless1, useless2, useless3 = get_emg_window_data(emg_data=data_angle, label_status=label_status, label_group=label_group, window_size=window_size_global,
                                                                   window_overlap_size=window_overlap_size_global)
    data_torque, useless1, useless2, useless3 = get_emg_window_data(emg_data=data_torque, label_status=label_status, label_group=label_group, window_size=window_size_global,
                                                                    window_overlap_size=window_overlap_size_global)

    data_details = [[subject] * np.size(data_group, 0), [movement] * np.size(data_group, 0), data_group]

    save_path = path_save_emg_windows_to_folder
    files_name = 'sub_' + str(subject) + '_mov_' + str(movement) + '_windows.mat'

    windows = {'windows': data_emg, 'angle_name': angle_name, 'angle_windows': data_angle, 'torque_name': torque_name, 'torque_windows': data_torque, 'num_labels': data_num_label, 'OneHot_labels': data_OneHot_label,
               'subject': data_details[0], 'movement': data_details[1], 'group': data_details[2]}

    scipy.io.savemat(save_path + files_name, windows)
...
def get_all_emg_windows(path_folder):
    ...
    for subject_name in subjects_list_global:
        for movement_check in range(movements_quantity_global):
            ...
            windows_path = path_folder + 'sub_' + str(subject_name) + '_mov_' + str(movement_name) + '_windows.mat'
            windows_data = scipy.io.loadmat(windows_path)

            windows_emg = list(windows_data['windows'][:])
            windows_angle = list(windows_data['angle_windows'][:])
            windows_torque = list(windows_data['torque_windows'][:])
            windows_num_labels = list(windows_data['num_labels'][0][:])
            windows_OneHot_label = list(windows_data['OneHot_labels'][:])
            windows_subject = list(windows_data['subject'][0][:])
            windows_movement = list(windows_data['movement'][:])
            windows_group = list(windows_data['group'][0][:])

            windows = windows + windows_emg
            angle_windows = angle_windows + windows_angle
            torque_windows = torque_windows + windows_torque
            num_labels = num_labels + windows_num_labels
            OneHot_labels = OneHot_labels + windows_OneHot_label
            details_subject = details_subject + windows_subject
            details_movement = details_movement + windows_movement
            details_group = details_group + windows_group
    ...
    movements_label_list = gait_period_global[:]
    ...
    outputs = {'examples': np.array(windows).tolist(), 'angle_name': angle_name, 'angle_labels': angle_labels, 'torque_name': torque_name, 'torque_labels': torque_labels,
...
def get_emg_standard(data_path):
    ...
    details_dic = {'subject': subject, 'group': group}
    ...
    outputs = {'examples': emg_standard, 'num_labels': num_labels, 'OneHot_labels': OneHot_labels, 'angle_name': angle_name, 'angle_labels': angle_labels, 'torque_name': torque_name, 'torque_labels': torque_labels,
               'details_subject': subject, 'details_movement': movement, 'details_group': group,
               'movements_label_list': movements_label_list, 'num_label_list': num_label_list, 'OneHot_label_list': OneHot_label_list}
...
def get_features(data_path):
    ...
    outputs = {'examples': features_examples, 'num_labels': num_labels, 'OneHot_labels': OneHot_labels, 'angle_name': angle_name, 'angle_labels': angle_labels, 'torque_name': torque_name, 'torque_labels': torque_labels,
               'details_subject': subject, 'details_movement': movement, 'details_group': group,
               'movements_label_list': movements_label_list, 'num_label_list': num_label_list, 'OneHot_label_list': OneHot_label_list}
...
def get_features_standard(data_path):
    ...
    outputs = {'examples': features_standard, 'num_labels': num_labels, 'OneHot_labels': OneHot_labels, 'angle_name': angle_name, 'angle_labels': angle_labels, 'torque_name': torque_name, 'torque_labels': torque_labels,
               'details_subject': subject, 'details_movement': movement, 'details_group': group,
               'movements_label_list': movements_label_list, 'num_label_list': num_label_list, 'OneHot_label_list': OneHot_label_list}
...
global num_label_list_global, OneHot_label_list_global
num_label_list_global, OneHot_label_list_global = get_label_from_target_list(target_list=gait_period_global[:], label_type=label_type_global)
...
if __name__ == '__main__':
    ...
    if flag_load_emg_data == 1:
        ...
        flag_first_task = 0
        data_information, data_information_name = get_data_information(path_data_set_folder=path_data_set_folder_global[:], subjects_list=subjects_list_global[:], movements_list=movements_list_global[:])
